...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    After the durable write, OpenClaw claims and processes updates through the core channel-ingress drain (per-chat/per-topic lanes, complete at turn adoption, 10-minute pre-adoption stall timeout by default). Slow agent turns do not hold Telegram's delivery ACK. Set `OPENCLAW_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS` to override that timeout for unusually slow local providers.
