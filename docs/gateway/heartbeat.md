...
- Default main-session heartbeat runs keep the normal prompt shape when cache retention is viable (`cacheRetention: "short"` with intervals under 5 minutes, or `cacheRetention: "long"` with intervals under 60 minutes).
- Large routine main-session heartbeats are auto-isolated when cache viability is missing or expired, so they do not rebuild a large provider cache just to say `HEARTBEAT_OK`.
- If `isolatedSession: false` is set explicitly, OpenClaw respects that choice; large non-actionable heartbeats can be skipped with `reason=full-context-heartbeat-guard` instead of paying for a full-context cache write.
- Exec-completion, cron-event, and manual wake heartbeats are treated as actionable and are allowed through the guard.
