...
- `providerPayloadLog.enabled`: log final provider request payloads, assistant responses, and usage snapshots for deep prompt/cache audits (default: `false`). This redacts credential-like fields and binary image bytes, but prompt and response text can still be sensitive.
...
- `providerPayloadLog.includeRequest` / `includeResponse` / `includeUsage`: control which payload-log events are written (all default: `true`).
- `OPENCLAW_PROVIDER_PAYLOAD_LOG=1`: one-off env toggle for provider payload logging. `OPENCLAW_PROVIDER_PAYLOAD_LOG_FILE`, `OPENCLAW_PROVIDER_PAYLOAD_LOG_REQUEST`, `OPENCLAW_PROVIDER_PAYLOAD_LOG_RESPONSE`, and `OPENCLAW_PROVIDER_PAYLOAD_LOG_USAGE` override the matching fields. Legacy `OPENCLAW_ANTHROPIC_PAYLOAD_LOG=1` remains Anthropic-only.
