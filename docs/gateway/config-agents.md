  ...
  agents: {
    defaults: {
      heartbeat: {
        ...
        consumeMemoryPrepend: true, // default: true; let isolated turns consume queued associative recall
      },
    },
  },
}
...
- The heartbeat object is strict. Its supported fields are `agentId`, `every`, `activeHours`, `model`, `session`, `target`, `directPolicy`, `to`, `accountId`, `prompt`, `timeoutSeconds`, `lightContext`, `isolatedSession`, and `consumeMemoryPrepend`.
...
- `consumeMemoryPrepend`: when true (default), an isolated heartbeat may claim queued associative-recall context. Set false when heartbeat work must not consume recall intended for the next interactive turn.
- Large-session cost guard: a routine heartbeat with a fresh main-session token count of at least 50,000 stays on the canonical main dialogue only when `isolatedSession` and `lightContext` are false/unset, the effective cache retention outlives the cadence, and this process has provider-confirmed proof of a covering one-hour write or refresh. The exact stable prompt, tools, endpoint, model, transport, and credential fingerprint must still match before provider I/O. First-party Anthropic (`api.anthropic.com`) canonical-main turns default to one-hour retention; configured custom endpoints require explicit long retention. The admitted cache-keeper turn is pinned to that model and auth profile without configured model fallbacks. Otherwise, an unset `isolatedSession` automatically uses an isolated session with lightweight bootstrap context; explicit `isolatedSession: false` skips the routine run instead of replaying the large prompt. Isolated heartbeat runs use a one-shot short cache retention when the main agent uses long retention; explicit `none` remains uncached. Manual heartbeats and runs with cron events, exec completions, scheduled tasks, or due commitments are not redirected by this guard.
