...
- Large-session cost guard: a routine heartbeat with a fresh main-session token count of at least 50,000 automatically uses an isolated session and lightweight bootstrap context when `isolatedSession` is unset. Setting `isolatedSession: false` opts out of automatic isolation; the same routine run is skipped instead of replaying the large main-session prompt. Manual heartbeats and runs with cron events, exec completions, scheduled tasks, or due commitments are not redirected by this guard.
