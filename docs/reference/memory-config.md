...
sqlite-vec query and exact-cosine fallback both run in bounded, killable child
processes so large scans cannot block the Gateway event loop. Worker failure
makes that memory search fail explicitly rather than retrying the scan in the
Gateway process.
