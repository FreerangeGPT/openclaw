...
- Keep heartbeat below your TTL only for agents that benefit from warm caches. Provider cache
  misses can still occur below the nominal TTL, so use `cacheRead` and `cacheWrite` as the
  operational signal.
