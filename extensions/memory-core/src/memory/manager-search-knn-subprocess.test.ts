...
function insertVectorRow(
  ...
  db.prepare(
    "INSERT INTO memory_index_chunks (id, path, start_line, end_line, text, embedding, source, model) VALUES (?, ?, 1, 1, ?, ?, ?, ?)",
  ).run(
    params.id,
...
describe("memory vector KNN subprocess boundary", () => {
  ...
  it("runs the exact-cosine fallback in the bounded child process", async () => {
    const fixture = await createFileBackedVectorDatabase();
    try {
      insertVectorRow(fixture.db, { id: "memory-hit", source: "memory", vector: [1, 0] });
      insertVectorRow(fixture.db, { id: "session-hit", source: "sessions", vector: [0.9, 0.1] });

      const result = await runVectorExactInSubprocess({
        databasePath: fixture.databasePath,
        request: {
          providerModels: ["test-model"],
          queryVec: [1, 0],
          limit: 2,
          snippetMaxChars: 700,
          sourceFilter: buildMemorySourceFilter(undefined, ["memory"]),
        },
      });

      expect(result.map((row) => row.id)).toEqual(["memory-hit"]);
