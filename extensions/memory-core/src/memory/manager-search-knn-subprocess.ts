...
import type {
  VectorExactChildInput,
  VectorKnnChildInput,
  VectorKnnChildResult,
  VectorSearchChildInput,
} from "./manager-search-knn.child.js";
import {
  isVectorKnnRow,
  type VectorKnnRequest,
  type VectorKnnResponse,
} from "./manager-search-knn.js";
import type { SearchRowResult, VectorExactRequest } from "./manager-search.js";
...
function parseChildEnvelope(output: Buffer): VectorKnnChildResult {
  let parsed: unknown;
  try {
...
function parseKnnChildResult(output: Buffer, maxRows: number): VectorKnnResponse {
  const result = parseChildEnvelope(output);
  const value = result.status === "ok" && !Array.isArray(result.value) ? result.value : undefined;
  if (
    !value ||
    !Array.isArray(value.rows) ||
    value.rows.length > maxRows ||
    value.rows.some((row) => !isVectorKnnRow(row)) ||
...
function isSearchRowResult(value: unknown): value is SearchRowResult {
  if (!value || typeof value !== "object") {
    return false;
  }
  const row = value as Partial<SearchRowResult>;
  return (
    ...
    (row.source === "memory" || row.source === "sessions")
  );
}

function parseExactChildResult(output: Buffer, maxRows: number): SearchRowResult[] {
  const result = parseChildEnvelope(output);
  if (
    result.status !== "ok" ||
    !Array.isArray(result.value) ||
    result.value.length > maxRows ||
    result.value.some((row) => !isSearchRowResult(row))
  ) {
    throw new VectorKnnSubprocessError(
      "memory vector exact child returned an invalid result",
      "protocol",
    );
  }
  return result.value as SearchRowResult[];
}
...
type VectorExactSubprocessParams = {
  databasePath: string;
  request: VectorExactRequest;
  signal?: AbortSignal;
};

type VectorSearchSubprocessParams<T> = {
  input: VectorSearchChildInput;
  signal?: AbortSignal;
  parseResult: (output: Buffer) => T;
};

async function runVectorSearchInSubprocess<T>(params: VectorSearchSubprocessParams<T>): Promise<T> {
  if (params.signal?.aborted) {
    throw toAbortError(params.signal);
  }
  const inputPayload = Buffer.from(JSON.stringify(params.input), "utf8");
  ...
  return await new Promise<T>((resolve, reject) => {
    ...
    child.once("close", (code, signal) => {
      ...
      settleCaller(() => {
        ...
        try {
          resolve(params.parseResult(Buffer.concat(stdoutChunks)));
        } catch (error) {
          reject(
...
export async function runVectorKnnInSubprocess(
  ...
  return await runVectorSearchInSubprocess({
    input,
    signal: params.signal,
    parseResult: (output) => parseKnnChildResult(output, params.request.limit),
  });
}

/** Run the exact-cosine fallback off the Gateway event loop under the same hard limits. */
export async function runVectorExactInSubprocess(
  params: VectorExactSubprocessParams,
): Promise<SearchRowResult[]> {
  const input: VectorExactChildInput = {
    kind: "exact",
    databasePath: params.databasePath,
    request: params.request,
  };
  return await runVectorSearchInSubprocess({
    input,
    signal: params.signal,
    parseResult: (output) => parseExactChildResult(output, params.request.limit),
  });
}
