...
export type VectorExactChildInput = {
  kind: "exact";
  databasePath: string;
  request: VectorExactRequest;
};

export type VectorSearchChildInput = VectorKnnChildInput | VectorExactChildInput;

export type VectorKnnChildResult =
  | { status: "ok"; value: VectorKnnResponse | SearchRowResult[] }
  | { status: "failed"; error: string };

function isChildInput(value: unknown): value is VectorSearchChildInput {
  if (!value || typeof value !== "object") {
    return false;
  }
  // SAFETY: the object guard above permits explicit validation of every field read below.
  const input = value as Partial<VectorSearchChildInput>;
  return (
    (input.kind === "knn" || input.kind === "exact") &&
...
async function run(input: unknown): Promise<VectorKnnChildResult> {
  ...
  try {
    ...
    if (input.kind === "exact") {
      return { status: "ok", value: await searchChunksByEmbedding(db, input.request) };
    }
