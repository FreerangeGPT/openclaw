...
describe("resolveTelegramAdoptionStallTimeoutMs", () => {
  it("allows ten minutes for polling adoption by default", () => {
    expect(resolveTelegramAdoptionStallTimeoutMs({ env: {} })).toBe(10 * 60 * 1_000);
  });

  it("keeps explicit and environment overrides", () => {
    expect(resolveTelegramAdoptionStallTimeoutMs({ configured: 12_345, env: {} })).toBe(12_345);
    expect(
      resolveTelegramAdoptionStallTimeoutMs({
        env: { OPENCLAW_TELEGRAM_SPOOLED_HANDLER_TIMEOUT_MS: "23456" },
      }),
    ).toBe(23_456);
  });
});
