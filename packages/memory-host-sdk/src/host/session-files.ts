...
function stripMemoryIndexExcludedPrefix(
  ...
  // Trusted ingress stamps model-only context; retain it in the transcript but
  // remove it from every lexical/vector memory index.
...
export async function buildSessionEntry(
  ...
  try {
    ...
    for (
      ...
      if (
        (insideHeartbeatTurn && !includedHeartbeatText) ||
