...
export async function handleAgentExecutionError(params: {
  ...
  const activeSessionEntry = turn.getActiveSessionEntry();
  const replayPromptTokens =
    activeSessionEntry?.totalTokensFresh === true ? activeSessionEntry.totalTokens : undefined;
  const expensiveReplayDecision = shouldDampenExpensiveReplay({
    promptTokens: replayPromptTokens,
  });
  const suppressExpensiveOverloadRetry =
    isOverloaded &&
    !params.overloadRetryState.unsafeToReplay &&
    params.overloadRetryState.retryCount < MAX_OVERLOAD_RETRIES &&
    expensiveReplayDecision.dampen;
  if (suppressExpensiveOverloadRetry) {
    defaultRuntime.error(
      `Overloaded provider before reply (${sanitizeForLog(message)}). ` +
        `Suppressing automatic replay at ${expensiveReplayDecision.promptTokens} prompt tokens.`,
    );
  }
  ...
  if (
    isOverloaded &&
    !params.overloadRetryState.unsafeToReplay &&
    !suppressExpensiveOverloadRetry &&
    params.overloadRetryState.retryCount < MAX_OVERLOAD_RETRIES
  ) {
    ...
    !params.shouldSurfaceToControlUi
      ? buildExternalRunFailureReply(
          { message, error: err },
          {
            ...
            replayPrevented:
              params.overloadRetryState.unsafeToReplay || suppressExpensiveOverloadRetry,
          },
        )
      : undefined;
  const fallbackText = isBilling
    ...
    : suppressExpensiveOverloadRetry
      ? formatExpensiveReplayMessage(expensiveReplayDecision)
      : isRateLimit && !isOverloaded
