...
import { resolveCacheRetention } from "../agents/embedded-agent-runner/prompt-cache-retention.js";
import { formatReasoningMessage } from "../agents/embedded-agent-utils.js";
import { resolveAgentHarnessPolicy } from "../agents/harness/policy.js";
import { modelKey, resolveModelRefFromString, type ModelRef } from "../agents/model-selection.js";
...
function resolveHeartbeatCacheRetention(params: {
  ...
  const defaults = resolveDefaultModel({ cfg: params.cfg, agentId: params.agentId });
  const heartbeatModel = normalizeOptionalString(params.heartbeat?.model)
    ? (resolveModelRefFromString({
        cfg: params.cfg,
        raw: params.heartbeat?.model ?? "",
        defaultProvider: defaults.defaultProvider,
        aliasIndex: defaults.aliasIndex,
  ...
  const agentParams =
    params.agentId !== undefined
      ? resolveAgentConfig(params.cfg, params.agentId)?.params
      : undefined;
...
export async function runHeartbeatOnce(opts: {
  ...
  const autoIsolatedMainSession = shouldAutoIsolateMainSessionHeartbeat({
    ...
    isManualReason: opts.source === "manual",
    isWakeReason: preflight.isWakePayload,
  });
  ...
  const expensiveMainSessionHeartbeat = shouldSkipExpensiveMainSessionHeartbeat({
    ...
    hasHeartbeatInstructions: Boolean(
      preflight.heartbeatFileContent?.trim() && dueHeartbeatTasks.length === 0,
    ),
    hasTasks: dueHeartbeatTasks.length > 0,
    isCronEventReason: preflight.isCronWake,
    isExecEventReason: preflight.isExecEventWake,
    isManualReason: opts.source === "manual",
    isWakeReason: preflight.isWakePayload,
    ...
    if (preparedMemoryPrepend.block) {
      ...
      if (!commitResult.applied && commitResult.reason !== "noop") {
        log.warn("heartbeat: memory prepend queue commit skipped", {
          ...
          path: preparedMemoryPrepend.queuePath,
        });
      }
    }
