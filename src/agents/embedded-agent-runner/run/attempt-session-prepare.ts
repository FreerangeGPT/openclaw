...
export async function prepareEmbeddedAttemptSessionManager(input: {
  ...
  await input.withOwnedTranscriptWrite(async () => {
    ...
    if (attempt.promptCacheKeeperEvidenceId) {
      ...
      // Admission observed the prior leaf before persisting the keeper turn.
      // Under transcript ownership, that exact turn must be its only child.
      try {
        assertMainSessionCacheKeeperTranscriptAnchor({
          currentLeaf: leaf
            ? {
          ...
          persistedUserMessageId:
            attempt.userTurnTranscriptRecorder?.getAdmissionReceipt()?.entryId,
        });
      } catch (error) {
        ...
        // Settlement later synchronizes the in-memory session created from this manager.
